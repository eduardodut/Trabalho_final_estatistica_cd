...
class Simulador():
    ...
    def iterar(self):
        ...
        self.lista_infectados_tipo_1 = []
        self.lista_infectados_tipo_2 = []
        matriz_infectantes = self.matriz_status[(self.matriz_status == 1) + (self.matriz_status == 2)]

        indices_infectantes = zip(*matriz_infectantes.nonzero())
        
        for indice in indices_infectantes:
            if self.matriz_status[indice[0], indice[1]] == self.INFECTADO_TIPO_1:
                self.lista_infectados_tipo_1.append(indice)
            if self.matriz_status[indice[0], indice[1]] == self.INFECTADO_TIPO_2:
                self.lista_infectados_tipo_2.append(indice)
                
        dict = {
                'num_sadios': self.populacao_inicial -len(self.lista_infectados_tipo_1)- len(self.lista_infectados_tipo_2)-self.num_curados-self.num_mortos,
