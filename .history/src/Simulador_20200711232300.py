...
class Simulador():
    ...
    def iterar(self):
        ...
        lista_curados_t2 = self.checagem_cura_lista([indice for indice in self.lista_infectados_tipo_2 if indice not in lista_mortos])
        
        #atualiza a lista de infectados após infecção, morte e cura dos individuos
        self.lista_infectados_tipo_1 = [indice for indice in self.lista_infectados_tipo_1 if indice not in lista_curados_t1]
        self.lista_infectados_tipo_2 = [indice for indice in self.lista_infectados_tipo_2 if indice not in lista_curados_t2 and indice not in lista_mortos]
        ...
        #altera a lista para guardar as novas localizações            
        ...
        self.num_mortos = self.num_mortos + len(lista_mortos)
        num_infectados_t1 = len(self.lista_infectados_tipo_1)
        num_infectados_t2 = len(self.lista_infectados_tipo_2)
        self.num_curados = self.num_curados + len(lista_curados_t1) + len(lista_curados_t2)
        num_sadios = self.populacao_inicial - num_infectados_t1 - num_infectados_t2 - self.num_curados - self.num_mortos

        dict = {
                'num_sadios': num_sadios,
                'num_infect_t1': num_infectados_t1,
                'num_infect_t2': num_infectados_t2,
                'num_curados': self.num_curados,
                'num_mortos': self.num_mortos}
       
        self.dataframe = self.dataframe.append(dict, ignore_index=True)    

        #cria os curados no grid
        for indice in lista_curados_t1 + lista_curados_t2:
            self.criar_individuo(self.CURADO,indice)

        #cria os mortos no grid
        for indice in lista_mortos:
            self.criar_individuo(self.MORTO,indice)

        #cria os novos infectados tipo 1
        ...
        #cria os novos infectados tipo 2
        ...
        # #self.num_curados = self.num_curados + len(lista_curados_t1) + len(lista_curados_t2) 
        # self.num_mortos = np.sum(self.matriz_status == self.CURADO)
        ...
        #self.populacao_inicial - self.num_mortos - self.num_curados - len(self.lista_infectados_tipo_1) - len(self.lista_infectados_tipo_2)
