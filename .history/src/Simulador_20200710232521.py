...
class Simulador():
    ...
    def iterar(self):
        ...
        self.matriz_status = lil_matrix(self.dataframe)
...
while sim.dataframe.iloc[-1]['num_infect_t1']+sim.dataframe.iloc[-1]['num_infect_t2'] > 0:
    print(typeof(sim.lista_matrizes_posicionamento[len(sim.lista_matrizes_posicionamento)-1].toarray()))
    plt.matshow(sim.lista_matrizes_posicionamento[len(sim.lista_matrizes_posicionamento)-1].toarray(), cmap = cmap, vmin= 0, vmax = 4)
    #plt.show()
    sim.iterar()
